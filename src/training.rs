...
use burn::{config::Config, data::{dataloader::DataLoaderBuilder, dataset::transform::PartialDataset}, nn::loss::BinaryCrossEntropyLossConfig, optim::{GradientsParams, Optimizer, SgdConfig}, record::CompactRecorder, tensor::backend::AutodiffBackend, train::{metric::LossMetric, LearnerBuilder}};
...
//     let dataset = CustomDataset::load("data/train",30).unwrap();
...
//         .build(PartialDataset::new(dataset.clone(),0,20));

//     let dataloader_test = DataLoaderBuilder::new(batcher_valid)
//         .batch_size(config.batch_size)
//         .shuffle(config.seed)
//         .num_workers(config.num_workers)
//         .build(PartialDataset::new(dataset,21,29));
